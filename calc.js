...
$( '#shippingFeesCheckbox' ).click(() => {
  ...
  const additionalText = ` $10 x ${count}`;

  if (isChecked) {
    $( '#shippingFeesLabel' ).text($( '#shippingFeesLabel' ).text() + additionalText);
  } else {
...
$( '#stateCheckbox' ).click(() => {
  ...
  if (isChecked) {
    $( '#stateLabel' ).text($( '#stateLabel' ).text() + ' State');
